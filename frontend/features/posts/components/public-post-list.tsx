  ...
  return (
    <section className="relative mx-auto flex w-full max-w-7xl flex-col gap-8 px-4 py-10 md:px-8 md:py-14">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <h1 className="max-w-4xl font-serif text-5xl leading-none text-(--ink) md:text-7xl">
              Yayinlanmis icerigin sade public vitrini.
            </h1>
        ...
        <div className="rounded-2xl border border-[var(--line)] bg-[var(--panel)] px-5 py-4 text-sm text-(--ink-muted)">
      ...
      <form className="grid gap-3 rounded-2xl border border-[var(--line)] bg-[var(--panel)] p-4 md:grid-cols-[1fr_auto]">
        <input
          type="search"
          name="q"
          defaultValue={query}
          placeholder="Baslik veya icerikte ara"
          className="h-12 rounded-xl border border-[var(--line)] bg-[var(--panel-deep)] px-5 text-sm text-(--ink) outline-none transition placeholder:text-(--ink-muted) focus:border-[var(--accent)]"
        />
        <button
          type="submit"
          className="inline-flex h-12 items-center justify-center rounded-xl bg-[var(--accent)] px-6 text-sm font-semibold text-[var(--accent-foreground)] transition hover:bg-[var(--accent-strong)]"
        >
          Aramayi uygula
        </button>
      </form>
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {posts.items.map((post) => (
            <Link key={post.id} href={`/posts/${post.id}`} className="group block">
              <Card className="h-full overflow-hidden bg-[var(--panel)] transition duration-200 hover:border-[var(--line-strong)]">
      ...
      {posts.meta.totalPages > 1 ? (
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[var(--line)] bg-[var(--panel)] p-4">
          <Link
            href={buildPageHref(posts.meta.page - 1, query)}
            aria-disabled={posts.meta.page <= 1}
            className="inline-flex h-11 items-center justify-center rounded-xl border border-[var(--line)] bg-[var(--panel-deep)] px-5 text-sm font-semibold text-(--ink) transition hover:bg-[var(--panel-soft)] aria-disabled:pointer-events-none aria-disabled:opacity-40"
          >
            Onceki sayfa
          </Link>
          ...
          <Link
            ...
            className="inline-flex h-11 items-center justify-center rounded-xl bg-[var(--accent)] px-5 text-sm font-semibold text-[var(--accent-foreground)] transition hover:bg-[var(--accent-strong)] aria-disabled:pointer-events-none aria-disabled:opacity-40"
          >
            Sonraki sayfa
          </Link>
        </div>
